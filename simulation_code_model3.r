...
source("complement_functions.R")
source("RfunctionsAFP.R")
source("data_generation_enriched_sigmafixed.R")
Rcpp::sourceCpp("enriched_hamming_sigmafixed.cpp")
...
# Model parameters
K <- 3          # outer clusters
Si <- c(2,2,2)  # inner clusters per outer
SS <- sum(Si)
mm <- 4         # modalities per variable
p <- 15         # number of variables
nj <- rep(50,SS)# number of observations

# Sigma values for each inner cluster
sigma_values <- rep(0.3,K)
center_values <- c(1,1,3,3,2,2)
...
create_output_directory <- function(seed, p, sigma_values, totiter, burnin, n, 
                                  gamma_m, gamma_s, gmm_m, gmm_s, tao, center_values) {
  sigmas <- paste(sigma_values, collapse = "_")
  centers <- paste(center_values, collapse = "_")
  output_dir <- paste0("model3_v2analysis_sim_seed",seed,"_p",p,"_tao",tao,
                         "_sigma",sigmas,
                         "totiter",totiter+burnin,"_n",n,"_gamma_m",gamma_m,
                         "_gamma_s",gamma_s,"_gamma_tilda_m",gmm_m,"_gamma_tilda_s",gmm_s,"_centers",centers)
  folder_path <- paste0(getwd(),"/model3_v2analysis_sim_seed", seed,"_p",p,"_tao",tao,
                          "_sigma",sigmas,
                          "totiter",totiter+burnin,"_n",n,"_gamma_m",gamma_m,"_gamma_s",gamma_s,"_gamma_tilda_m",gmm_m,"_gamma_tilda_s",gmm_s,"_centers",centers)
...
initialize_parameters <- function(data_list, seed, K, SS, p, sigma_values, M_init = K, S_init = Si, n) {
    ...
    w_un <- rep(1, M)
    q_un <- rep(1, sum(S))
    s <- data_list[[seed]]$true_inner
    m <- data_list[[seed]]$groundTruth
    ...
    sigma <- matrix(0.4, nrow = p, ncol = M)
    center <- matrix(2, nrow = p, ncol = sum(S))
    zeta = matrix(1,nrow = p, ncol = M) 

  return(list(M = M, S = S, s = s, m = m, w_un = w_un, q_un = q_un, 
              sigma = sigma, center = center, zeta = zeta))
}
    ...
    for (gmm_s in gmm_s) {
      for (gamma_m in gamma_m) {
        for (gamma_s in gamma_s) {
          ...
          dirs <- create_output_directory(seed, p, sigma_values, totiter, burnin, sum(nj), 
                                        gamma_m, gamma_s, gmm_m, gmm_s, tao, center_values = center_values)
          output_dir <- dirs$output_dir
          folder_path <- dirs$folder_path
          
          # Generate data
          data_list[[seed]] <- ham_enr_gen(K, Si, p, nj, mm, sigma_values, center_values,
                                           folder_path, save = TRUE, seed=seed)
          ...
          set.seed(seed)
          init_params <- initialize_parameters(data_list, seed, K, Si, p, sigma_values, n = sum(nj))
          ...
          # Run C++ MCMC
          post_burnin_chain <- run_mcmc_sigmafixed_cpp(
            Y = Y_matrix,
            m_init = m,
            s_init = s,
            w_un_init = w_un,
          ...
          psm <- comp.psm(C)
          VI <- minVI(psm)
          rand_VI[[seed]] = mcclust::arandi(VI$cl,data_list[[seed]]$groundTruth,adjust = FALSE)
          ...
          # Save PSM plot
          png(filename = file.path(output_dir, "psm_outer_plot.png"), width = 800, height = 600)
          ...
          ###INNER STUDY
          C <- as.matrix(read.table(file.path(output_dir, "lower_clustering.csv"), 
                                    sep = ",", header = FALSE))
          psm <- comp.psm(C)
          VI_inner <- minVI(psm)
          rand_VI_inner[[seed]] = mcclust::arandi(VI_inner$cl,data_list[[seed]]$groundTruth,adjust = FALSE)
          
          png(filename = file.path(output_dir, "psm_inner_plot.png"), width = 800, height = 600)
          myplotpsm2(psm, classes = data_list[[seed]]$groundTruth , ax = FALSE, ay = FALSE)
          dev.off()
          ###
          ...
          ari_inner <- arandi(VI_inner$cl, data_list[[seed]]$groundTruth, adjust = FALSE)
          results_summary <- rbind(results_summary, data.frame(
            ...
            gamma_tilde_s = gmm_s,
            tao = tao,
            rand_outer = ari_value,
            rand_inner = ari_inner,
            PosteriorModeClusters = post_mode_clusters,
            clusters_VI = length(cluster_ids),
            clusters_inner = length(unique(VI_inner$cl))
          ))
        }
      }
    }
...
# Save final results
file_name <- paste0("summary_simulation_results_p",p,"_tao",tao,"_sigma",sigma_values[1],"_",sigma_values[2],"_",sigma_values[3],"totiter",totiter+burnin,"_n",sum(nj),".xlsx")
